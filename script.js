...
//getting the reponse and setting the text
        UVValue.text(UVIndex);
//create a paragraph to hold the UV value
        var uvIndexEl = $("<p class='card-text'>").text("UV Index: ");
//append the element to the value, then to the div
        UVValue.appendTo(uvIndexEl);
        weatherDiv.append(uvIndexEl);
        $("#weatherContainer").html(weatherDiv);
}

//function displays 5 day weather with Open Weather API
async function displayFiveDay() {

    var queryURL = "https://api.openweathermap.org/data/2.5/forecast?q="+cityName+"&units=imperial&appid=1e14f105f335994533ed9aca6312a5c8";

    var response = await $.ajax({
        url: queryURL,
        method: "GET"
      })
//creat a div to hold the forecast
      var forecastDiv = $("<div  id='fiveDayForecast'>");
//create a header
      var forecastHeader = $("<h5 class='card-header border-secondary'>").text("5 Day Forecast");
//append the header to the div
      forecastDiv.append(forecastHeader);
//create a div of bootstrap cards, one will be used for each day
      var fiveDayDiv = $("<div  class='card-deck'>");
//append the cards to the div
      forecastDiv.append(fiveDayDiv);
      
//create a for loop to get the 5 days
      for (i=0; i<5;i++){
          var forecastCard = $("<div class='card mb-3 mt-3'>");
          var fiveDayBox = $("<div class='card-body'>");
          var date = new Date();
          var val=(date.getMonth()+1)+"/"+(date.getDate()+i+1)+"/"+date.getFullYear();
          var forecastDate = $("<h5 class='card-title'>").text(val);
//append the date to the to 5 day card
        fiveDayBox.append(forecastDate);
//go through the loop to get the responses. Same code as displayWeather function
        var weatherIcon = response.list[i].weather[0].icon;
        console.log(weatherIcon);
        var displayWeatherIcon = $("<img src = http://openweathermap.org/img/wn/" + weatherIcon + ".png />");
...
//function to show weather for city in history is selected
